...
Write-Host ''
Write-Host 'ROOTED: CANONICAL GATE V6 — canonical DO $do$, EXECUTE $q$, FN bodies $fn$ (parse-safe / StrictMode-safe)' -ForegroundColor Cyan
...
foreach ($f in $files) {
  ...
  for ($i=0; $i -lt $arr.Count; $i++) {
    ...
    if ($trim -match '(?i)^\$q\$\s*;\s*$') {
      if ($execStack.Count -eq 0) { AddErr $path ($i+1) 'Found $q$; close without execute open.' }
      else {
        $top = $execStack.Pop()
        if ($top[0] -ne '$q$') { AddErr $path ($i+1) ('Execute opened with ' + $top[0] + ' at line ' + $top[1] + ' but closed with $q$; (normalize opener to execute $q$).') }
      }
      continue
    }
    ...
    # FN close bare ($fn$ or $$) — preferred before DO closes
    if ($trim -match '(?i)^(\$\$|\$fn\$)\s*$') {
      if ($fnStack.Count -eq 0) { continue }
      $top = $fnStack.Peek()
    ...
    # Anti-pattern: $fn$; (should not exist in V6)
    if ($trim -match '(?i)^\$fn\$\s*;\s*$') {
      AddErr $path ($i+1) 'Found $fn$; (function close tag must be bare $fn$ with statement semicolon after, not on tag line).'
      continue
    }
  }

  while ($execStack.Count -gt 0) { $top = $execStack.Pop(); AddErr $path $top[1] ('Execute open ' + $top[0] + ' not closed with $q$;') }
  ...
  if ($text -match '(?im)^\s*do\s+\$\$\s*$') { AddErr $path 1 'Found non-canonical DO opener (must be do $do$).' }
}
