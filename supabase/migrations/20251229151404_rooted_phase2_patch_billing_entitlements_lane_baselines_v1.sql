...
-- 0) Preconditions (hard fail early)
do $$
begin
...
-- 1) Ensure lane vocabulary (Phase 2 canonical set)
...
-- 2) Ensure entitlement_codes registry exists + contains keys referenced by policies
...
-- 3) Normalize lane entitlement requirements (stop mapping commerce/payments to premium)
update public.vertical_lane_policy
set requires_entitlement_code = 'commerce'
where lane_code = 'commerce_listings'
  and requires_entitlement_code in ('premium','premium_plus');
...
-- 4) Backfill baseline lane rows for EVERY canonical vertical (no missing rows)
...
-- IMPORTANT:
--   role is NULL for global applicability (passes role_check)
--   tier uses only free/premium/premium_plus (passes tier_check)
-- This defines policy; users are evaluated by role/tier via user_tiers + your user_has_entitlement().

-- FREE baseline (minimal)
...
-- PREMIUM baseline (commerce + payments + media + games)
insert into public.billing_entitlements
  (entitlement_key, product_key, role, tier, feature_flags, capabilities, metadata, is_active)
select x.entitlement_key, x.product_key, x.role, x.tier, x.feature_flags::jsonb, x.capabilities::jsonb, x.metadata::jsonb, true
from (values
...
-- PREMIUM_PLUS baseline (everything + b2b_bulk)
insert into public.billing_entitlements
  (entitlement_key, product_key, role, tier, feature_flags, capabilities, metadata, is_active)
select x.entitlement_key, x.product_key, x.role, x.tier, x.feature_flags::jsonb, x.capabilities::jsonb, x.metadata::jsonb, true
from (values
...
-- PACK EXAMPLES (CHECK-safe):
-- - b2b_bid: apply globally (role NULL), tier premium_plus (or premium). Here: premium_plus.
-- - b2g_procurement: institution only (role='institution'), tier premium_plus.
insert into public.billing_entitlements
  (entitlement_key, product_key, role, tier, feature_flags, capabilities, metadata, is_active)
select x.entitlement_key, x.product_key, x.role, x.tier, x.feature_flags::jsonb, x.capabilities::jsonb, x.metadata::jsonb, true
from (values
