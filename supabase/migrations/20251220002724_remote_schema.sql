...
drop policy "canonical_verticals_read_authenticated_v1" on "public"."canonical_verticals";

drop policy "group_capability_grants_read_authenticated_v1" on "public"."group_capability_grants";

drop policy "specialty_governance_group_members_read_authenticated_v1" on "public"."specialty_governance_group_members";

drop policy "specialty_governance_groups_read_authenticated_v1" on "public"."specialty_governance_groups";

drop policy "specialty_types_read_authenticated_v1" on "public"."specialty_types";

drop policy "vcs_read_authenticated_v1" on "public"."vertical_canonical_specialties";
...
alter table "public"."specialty_governance_group_members" drop constraint "specialty_governance_group_members_group_fkey";

alter table "public"."specialty_governance_group_members" drop constraint "specialty_governance_group_members_specialty_fkey";

alter table "public"."vertical_canonical_specialties" drop constraint "fk_vcs_vertical";

alter table "public"."specialty_types" drop constraint "specialty_types_pkey";

drop index if exists "public"."specialty_types_pkey";
...
alter table "public"."canonical_verticals" drop column "created_at";

alter table "public"."canonical_verticals" drop column "updated_at";
...
alter table "public"."specialty_governance_groups" drop column "updated_at";
...
alter table "public"."specialty_types" drop column "created_at";

alter table "public"."specialty_types" drop column "description";

alter table "public"."specialty_types" drop column "updated_at";
...
alter table "public"."vertical_canonical_specialties" drop column "created_at";

alter table "public"."vertical_canonical_specialties" drop column "updated_at";
...
BEGIN
  ...
  -- Ã¢Å“â€¦ Apply status change by entity type
...
begin
    ...
    -- 2) Enforce global result limits (6Ã¢â‚¬â€œ8 only)
...
BEGIN
  ...
  -- If it matches primary vertical, weÃ¢â‚¬â„¢re good
  IF NEW.vertical_code = v_primary THEN
    RETURN NEW;
  END IF;

  -- Otherwise, only allow if thereÃ¢â‚¬â„¢s a specialty overlay
  SELECT EXISTS (
    SELECT 1
...
BEGIN
  ...
  IF inst_vertical IS NULL THEN
    RAISE EXCEPTION
      'Institution % has no primary vertical defined Ã¢â‚¬â€œ RFQ cannot be saved.',
      NEW.institution_id
      USING ERRCODE = 'P0001';
  END IF;
...
begin
  ...
  ----------------------------------------------------------------------
  -- 1) Baseline Ã¢â‚¬Å“listedÃ¢â‚¬Â badges (applies to seeded and unverified)
  ----------------------------------------------------------------------
...
BEGIN
  ...
  FROM public.providers p
  WHERE p.id = NEW.provider_id;  -- Ã¢Â¬â€¦Ã¯Â¸Â CHANGE provider_id IF YOUR COLUMN IS NAMED DIFFERENTLY
